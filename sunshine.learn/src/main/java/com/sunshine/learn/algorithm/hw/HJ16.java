    ...
    private static void processData(int[] m, int[] v, int[] p, int[][] _v, int[][] _p) {
        ...
        for (List<Integer> list : groups.values()) {
            ...
            if (size == 1) {
                ...
                _p[index][1] = p[list.get(0)] * v[list.get(0)];// list.get(0)] * v[list.get(0)
                                                               // 记得审题，本题中说每件物品的价格与重要度的乘积的总和最大
                ...
                _p[index][3] = p[list.get(0)] * v[list.get(0)] + p[list.get(1)] * v[list.get(1)]
                        + p[list.get(2)] * v[list.get(2)];
